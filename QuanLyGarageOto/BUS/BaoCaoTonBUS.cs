...
namespace BUS
{
    public class BaoCaoTonBUS
    {
        ...
        public DataTable LayDtVTPT()//Lấy DataTable từ KHO
        {
            return BaoCaoTonDAO.Instance.LayDtVTPT();
        }
        ...
        public DataTable TaoBaoCaoTon(DateTime b)//Tạo và trả về 1 báo cáo tồn từ 1 DataTable trống.
        {
            DataTable a;
            a = KhoiTaoBaoCaoTon();
            DataTable dt = LayDtVTPT();
            foreach (DataRow row in dt.Rows)
            {
                a = ThemHangBaoCaoTon(a, int.Parse(row["MaVTPT"].ToString()), row["TenVTPT"].ToString(), LuongTonDauThang(int.Parse(row["MaVTPT"].ToString()), b.Month, b.Year), SoLuongPhatSinh(int.Parse(row["MaVTPT"].ToString()), b.Month, b.Year), LuongTonCuoiThang(int.Parse(row["MaVTPT"].ToString()), b.Month, b.Year));
            }
            return a;
        }
        ...
        public DataTable TruyXuatBaoCaoTon(DateTime a)//Truy xuất báo cáo tồn đã được lưu trữ từ Database.
        {
            ...
            foreach (DataRow row in dt.Rows)
            {
                b = ThemHangBaoCaoTon(b, int.Parse(row["MaVTPT"].ToString()), row["TenVTPT"].ToString(), int.Parse(row["TonDau"].ToString()), int.Parse(row["PhatSinh"].ToString()), int.Parse(row["TonCuoi"].ToString()));
            }
            return b;
        }
